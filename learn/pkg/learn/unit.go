...
// several questions. A unit typically also has a an optional quiz after two
...
// perfectly so that the unit is considered mastered.
//
...
// summary.
// The order is captured in the OrderedModels slice.
...
func (m *UnitModel) printBadgesHTML(buf *bytes.Buffer) error {
	for _, model := range m.OrderedModels {
		badge, ref, err := badgeURL(model, m.Filename)
		if err != nil {
			return err
		}
...
func badgeURL(m model, unitFilename string) (string, string, error) {
	...
	dir := filepath.Dir(unitFilename)
	relPath, err := filepath.Rel(dir, fname)
	if err != nil {
