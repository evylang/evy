...
type GraphicsRuntime struct {
	...
	transformX func(float64) float64
	...
	transformY func(float64) float64
}
...
func NewGraphicsRuntime() *GraphicsRuntime {
	rt := &GraphicsRuntime{
		...
		transformX: func(x float64) float64 { return 10 * x },
		transformY: func(y float64) float64 { return 1000 - 10*y },
		scale:      func(s float64) float64 { return 10 * s },
	}
	rt.Clear("white")
	return rt
}
...
func (rt *GraphicsRuntime) nonDefaultAttr() Attr {
	...
	if rt.attr.StrokeWidth == defaultAttr.StrokeWidth {
		attr.StrokeWidth = ""
	}
...
func (rt *GraphicsRuntime) nonDefaultTextAttr() TextAttr {
	...
	if rt.textAttr.FontWeight == defaultTextAttr.FontWeight {
		textAttr.FontWeight = ""
	}
	if rt.textAttr.FontStyle == defaultTextAttr.FontStyle {
		textAttr.FontStyle = ""
	}
	if rt.textAttr.FontSize == defaultTextAttr.FontSize {
		textAttr.FontSize = ""
	}
...
func (rt *GraphicsRuntime) Ellipse(x, y, radiusX, radiusY, rotation, _, _ float64) {
	...
	ellipse := Ellipse{
		CX:        ftoa(x),
		CY:        ftoa(y),
		RX:        ftoa(rt.scale(radiusX)),
		RY:        ftoa(rt.scale(radiusY)),
		Transform: transform,
	}
...
func (rt *GraphicsRuntime) Gridn(unit float64, color string) {
	...
	for i := float64(0); i <= 1000; i += unit {
		si := ftoa(i)
		hLine := &Line{X1: si, Y1: "0", X2: si, Y2: "1000"}
		vLine := &Line{X1: "0", Y1: si, X2: "1000", Y2: si}
		if lineCnt%5 == 0 {
			hLine.StrokeWidth = "2"
			vLine.StrokeWidth = "2"
		}
...
func (rt *GraphicsRuntime) Width(w float64) {
	rt.Push()
	rt.attr.StrokeWidth = ftoa(rt.scale(w))
}
...
func (rt *GraphicsRuntime) Font(props map[string]any) {
	...
	if size, ok := props["size"].(float64); ok {
		rt.textAttr.FontSize = ftoa(rt.scale(size))
	}
	if weight, ok := props["weight"].(float64); ok {
		rt.textAttr.FontWeight = ftoa(weight)
	}
...
func (rt *GraphicsRuntime) WriteSVG(w io.Writer) error {
	...
	encoder.Indent("", "  ")
	if err := encoder.Encode(rt.SVG); err != nil { //nolint: musttag
